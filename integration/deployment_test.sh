...
run_test() {
    ...
    try ${PEER_CMD} lifecycle chaincode createenclave -o ${ORDERER_ADDR} --peerAddresses "localhost:7051" --name auction_test
    ...
    #first call negated as it fails
    try_fail ${PEER_CMD} lifecycle chaincode createenclave -o ${ORDERER_ADDR} --peerAddresses "localhost:7051" --name wrong-cc-id
    try ${PEER_CMD} lifecycle chaincode createenclave -o ${ORDERER_ADDR} --peerAddresses "localhost:7051" --name echo_test
