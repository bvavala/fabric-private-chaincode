...
type Stub interface {
	...
	Create(string, []byte, []byte, []byte) ([]byte, error)
...
func (e *StubImpl) Create(enclaveLibFile string, ccParametersBytes []byte, attestationParametersBytes []byte, hostParametersBytes []byte) ([]byte, error) {
	...
	if ret := C.sgxcc_create_enclave(&eid, C.CString(enclaveLibFile), (*C.uint8_t)(C.CBytes(attestationParametersBytes)), C.uint32_t(len(attestationParametersBytes)), (*C.uint8_t)(C.CBytes(ccParametersBytes)), C.uint32_t(len(ccParametersBytes)), (*C.uint8_t)(C.CBytes(hostParametersBytes)), C.uint32_t(len(hostParametersBytes)), (*C.uint8_t)(credentialsBuffer), credentialsBufferMaxSize, &credentialsSize); ret != 0 {
