...
bool cc_data::build_attested_data(ByteArray& attested_data)
{
    ...
    {
        // fpc_Attested_Data_cc_params_tag
        COND2ERR(!pb_encode_tag(&ostream, PB_WT_STRING, fpc_Attested_Data_cc_params_tag));
        COND2ERR(!pb_encode_string(
            &ostream, (const unsigned char*)cc_parameters_.data(), cc_parameters_.size()));
    }

    {
        // fpc_Attested_Data_host_params_tag
        COND2ERR(!pb_encode_tag(&ostream, PB_WT_STRING, fpc_Attested_Data_host_params_tag));
        COND2ERR(!pb_encode_string(
            &ostream, (const unsigned char*)host_parameters_.data(), host_parameters_.size()));
    }

    {
        // fpc_Attested_Data_enclave_vk_tag
        std::string s = verification_key_.Serialize();
        COND2ERR(!pb_encode_tag(&ostream, PB_WT_STRING, fpc_Attested_Data_enclave_vk_tag));
